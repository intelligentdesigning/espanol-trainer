...
/**
 * Imperfecto de indicativo. Almost fully regular — only ser/ir/ver are irregular,
 * with NO stem changes or irregular yo-forms in this tense.
 * @param {string} infinitive
 * @returns {string[]|null}
 */
const IMPERFECT_IRREGULAR = {
  ser: ["era", "eras", "era", "éramos", "erais", "eran"],
  ir: ["iba", "ibas", "iba", "íbamos", "ibais", "iban"],
  ver: ["veía", "veías", "veía", "veíamos", "veíais", "veían"],
};
export function conjugateImperfectoAll(infinitive) {
  const inf = infinitive.trim().toLowerCase();
  if (IMPERFECT_IRREGULAR[inf]) return IMPERFECT_IRREGULAR[inf].slice();
  const end = ending(inf);
  if (end !== "ar" && end !== "er" && end !== "ir") return null;
  const stem = inf.slice(0, -2);
  const suf = end === "ar"
  ...
  return suf.map((s) => stem + s);
}

/** Tenses the engine can produce, in display order (used by the data pipeline). */
export const TENSE_BUILDERS = {
  presente: conjugatePresentAll,
  imperfecto: conjugateImperfectoAll,
};
